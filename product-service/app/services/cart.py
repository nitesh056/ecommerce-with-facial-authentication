...
async def get_cart(user_id: int):
    cart = await Cart.get_or_none(user_id=user_id, status='saved').prefetch_related('cart_items')
    if cart:
        return await Cart_Pydantic.from_tortoise_orm(cart)
